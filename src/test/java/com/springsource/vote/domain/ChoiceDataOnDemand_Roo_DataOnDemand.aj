...
privileged aspect ChoiceDataOnDemand_Roo_DataOnDemand {
    ...
    public Choice ChoiceDataOnDemand.getNewTransientChoice(int index) {
        com.springsource.vote.domain.Choice obj = new com.springsource.vote.domain.Choice();
    ...
    public void ChoiceDataOnDemand.init() {
        data = com.springsource.vote.domain.Choice.findChoiceEntries(0, 10);
        if (data == null) throw new IllegalStateException("Find entries implementation for 'Choice' illegally returned null");
        if (!data.isEmpty()) {
            return;
        }
        
        data = new java.util.ArrayList<com.springsource.vote.domain.Choice>();
        for (int i = 0; i < 10; i++) {
            com.springsource.vote.domain.Choice obj = getNewTransientChoice(i);
            obj.persist();
            obj.flush();
            data.add(obj);
        }
