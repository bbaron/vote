...
privileged aspect VoteDataOnDemand_Roo_DataOnDemand {
    ...
    public Vote VoteDataOnDemand.getNewTransientVote(int index) {
        com.springsource.vote.domain.Vote obj = new com.springsource.vote.domain.Vote();
        obj.setChoice(null);
        obj.setIp("ip_" + index);
    ...
    public void VoteDataOnDemand.init() {
        data = com.springsource.vote.domain.Vote.findVoteEntries(0, 10);
        if (data == null) throw new IllegalStateException("Find entries implementation for 'Vote' illegally returned null");
        if (!data.isEmpty()) {
            return;
        }
        
        data = new java.util.ArrayList<com.springsource.vote.domain.Vote>();
        for (int i = 0; i < 10; i++) {
            com.springsource.vote.domain.Vote obj = getNewTransientVote(i);
            obj.persist();
            obj.flush();
            data.add(obj);
        }
